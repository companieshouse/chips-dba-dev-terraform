...
locals {
  ...
  chips-staging= values(data.vault_generic_secret.chips_staging.data)
  ...
  oracle_allowed_ranges = concat(local.internal_cidrs, var.vpc_sg_cidr_blocks_oracle, local.dev_data_cidrs, local.dev_application_cidrs, local.chs_application_cidrs, local.cdp_dev_data_cidrs)
